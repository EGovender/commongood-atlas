...
`site/src/pages/design/logical-model.astro` + `site/src/components/logical-model/` (`LogicalModelWorkbench.tsx`/`LogicalModelSummary.tsx`/`LogicalModelList.tsx`/`LogicalModelDiagram.tsx`/`LogicalModelInspector.tsx`) mirror the Conceptual Model workspace's own component split (including its Diagram/List view toggle, defaulting to Diagram), reached via an in-flow "Logical model" link from `/design/model/`'s header (not a persistent top-level nav entry, matching `/design/model/` itself). `buildLogicalModelMermaid()` still emits a real Mermaid `erDiagram` block alongside the in-browser diagram, for viewing/pasting outside the site.

`LogicalModelDiagram.tsx` is a genuinely new SVG/D3 node renderer, not a reuse of `nodeShapePath()` — `GraphExplorer`/`ConceptualModelDiagram`'s node shapes are a single fixed-size symmetric glyph per concept, with no notion of internal rows or variable height, which a boxy multi-attribute-row entity fundamentally needs. `site/src/data/entity-shapes.ts` holds the new pure layout/geometry math (unit-tested independently of D3/React, same discipline as `graph-utils.ts`):

- `computeEntityBoxLayout(attributeCount)` — every entity box shares one fixed width; only height (and each attribute row's y-offset) grows with attribute count, so an entity like Use Restriction (13+ attributes) renders a visibly taller box than one with two or three.
- `intersectRayWithBox(dx, dy, halfWidth, halfHeight)` — where a line from a box's center exits its rectangular boundary, replacing the circular pullback math a symmetric bubble node could get away with.
- `truncateLabel(text, maxChars)` — plain SVG `<text>` has no CSS ellipsis/wrapping, and `<foreignObject>` was deliberately avoided (risk of inconsistent rasterization when `exportSvgAsPng` loads the serialized SVG into an `Image` for the PNG export) — a simple character-budget truncation instead, tuned by eye rather than measured pixel-exactly.

Everything else -- the build-on-`[model]`/classed-highlight-on-`[selectedId]` effect split, zoom/drag/reduced-motion/export-PNG/toolbar machinery, and `seededGridPositions`/`getCategory`/`exportSvgAsPng` reuse -- is structurally identical to `ConceptualModelDiagram.tsx`, just re-tuned (bigger `LINK_DISTANCE`, more negative charge, larger seed spacing) for much bigger nodes. Two deliberate simplifications, documented rather than engineered around: **entity/entity collision uses a circumscribed-circle radius per node** (each box's own half-diagonal, not true rectangle-rectangle collision), and **association edges carry no cardinality notation** at their endpoints -- every business association's cardinality is `'unspecified'` today (a Logical Model milestone decision, not a diagram limitation), so drawing crow's-foot notation would fabricate precision the ontology doesn't have; edges keep a single midpoint label instead, same as the Conceptual Model diagram.
